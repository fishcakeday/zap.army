<script>
    ...
    $: {
        const cutoffTime = Math.floor(Date.now() / 1000) - 15 * 60;
        let sortedZaps = $zaps
...
{#each visibleZaps.slice(0, 10) as zap, index}
    <Zap {zap} {index} opened={openedZap === zap.id} on:open={(e) => { openedZap = e.detail } } />
{/each}